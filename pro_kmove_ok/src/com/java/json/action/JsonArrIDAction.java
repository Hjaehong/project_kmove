...
public class JsonArrIDAction implements Command {
	...
	public String actionDo(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		GroupEnterBoardDto getsub = new GroupEnterBoardDto();
		
		getsub = GroupEnterBoardDao.getInstance().toGetSub(board_number); // 누른 게시글의 제목
		String sub = getsub.getSubject();
		TrigDao.getInstance().inSub(board_number, sub); // 게시글의 제목을 해당 행에 update
		logger.info(logMsg+id+", board_number : "+board_number);
		
		

		int dont = GroupEnterBoardDao.getInstance().dontPeople(pushboard); //인원 꽉찻는지 검사
